...
sk.merge = require('merge-recursive');
require('util').inherits(sk, require('events').EventEmitter);

sk.prototype.addQueue = function(queue, item) {
  var self = this;
  if(typeof self._queue[queue] != []) self._queue[queue] = [];
  self._queue[queue].push(new queueItem(self, arguments));
  if(typeof self._queue.id == 'undefined' || self._queue.id === null) {
    self._queue.id = setImmediate(this.processQueue.bind(self));
  }
  return self;
}
...
sk.prototype.addIndex = function(field, callback, force) {
  ...
  if(!force) {
    ...
    //process.nextTick(self.processQueue.bind(self));
    self.addQueue('addIndex', new queueItem(self, arguments));
    return self; //QUEUED
  }
...
sk.prototype.insert = function(data, callback, force) {
  ...
  if(!force) {
    ...
    //process.nextTick(self.processQueue.bind(self));
    //if(self._queue.id !== null) setImmediate(self.processQueue.bind(self));
    self.addQueue('insert', new queueItem(self, arguments));
    return self; //QUEUED;
  }
...
sk.prototype.update = function(where, data, opts, callback, force) {
  var self = this;
  force = force || ((arguments.length > 1 && typeof arguments[arguments.length - 1] == 'boolean') ? arguments[arguments.length - 1] : false);

  if(!force) {
    self.addQueue('update', new queueItem(self, arguments));
    return self;
  }
  opts = opts || {};
  opts.merge = opts.merge || self.merge || sk.merge

  return self;
}; // end update

sk.prototype.remove = function(where, callback, force) {
  var self = this;
  if(!force) {
    self.addQueue('update', new queueItem(self, arguments));
    return self;
...
sk.prototype.find = function(where, callback, force) {
  var self = this;
  if(!force) {
    self.addQueue('update', new queueItem(self, arguments));
    return self;
