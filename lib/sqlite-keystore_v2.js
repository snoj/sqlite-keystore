...
var sk = module.exports = function(file, opts) {
  ...
  if(typeof file == 'string') {
    this.db = new sqlite.Database(file, opts.mode, function(err) {
      ...
      self.db.run("PRAGMA journal_mode=PERSIST;");
  ...
  Object.defineProperty(this, "_queue", {
    enumerable: false
    ,writable: true
    ,value: {mustQueue: false, processing: false, addIndex: [], insert: [], update: [], remove: [], find: []}
  });
...
sk.prototype.processQueue = function(queue) {
  ...
  for(var i = 0; i < queues.length; i++) {
    this.db.parallelize(function() {
      var f = queues[i];
      /*for(var j = 0; j < this._queue[f].length; j++) {
        this[f].apply(this, this._queue[f][i]);
      }*/
